  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-emerald-400 to-green-200 p-6">
      <div className="bg-white rounded-3xl shadow-xl p-10 max-w-lg w-full text-center space-y-6">
        <h1 className="text-4xl font-bold text-gray-800">Crushed it!</h1>
        ...
        {submitted !== "submitted" ? (
          <>
            ...
            <p>
              If you add the number of letters in Daksh and Sierra's middle
              names. Multiply that by the distance in miles between their first
              address together and their current address. Divide that number by
              the total years Daksh and Sierra were in college. Closest to
              correct number will win the tie breaker!
            </p>
            {error && (
              <p className="text-red-400">
                Please add both name and a tiebreaker guess!
              </p>
            )}
            <input
              ...
              className="p-1 border-1 rounded-lg w-full"
            />
            <input
              placeholder="Guess"
              onChange={(e) => {
            ...
            />
            <button
              className="mt-4 border-1 border-aqua-200 py-2 w-full rounded-full text-contrast-text text-bold text-lg "
        ...
        <div className="text-3xl animate-bounce space-x-2">🎊 🧖‍♀️ 🧖‍♂️ 🎊</div>
      </div>
    </div>
  );
};
